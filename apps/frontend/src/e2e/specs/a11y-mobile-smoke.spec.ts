...
async function assertMinTappable(
  locator: ReturnType<typeof test.extend>['arguments'][0]['page']['locator'],
  min = 44
) {
  const box = await locator.boundingBox();
  expect(
    box,
    `Element ${await locator.toString()} should be attached with a bounding box`
  ).not.toBeNull();
  if (!box) return;
...
test.describe('A11y & Mobile smoke', () => {
  ...
  test('critical controls are visible, tappable, and page has no critical a11y violations', async ({
    page,
  }) => {
