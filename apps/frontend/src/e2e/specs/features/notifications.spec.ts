...
test.describe('Add Asset Notifications', () => {
  ...
  test('shows success notification when adding new asset', async ({ page }) => {
    ...
    const toast = page.locator('[role="status"]', {
      hasText: /Added 1.5 BTC to your portfolio/i,
    });
  ...
  test('shows different message when adding to existing asset', async ({
    page,
  }) => {
    await page.goto('/portfolio');
...
test.describe('Delete Asset Notifications', () => {
  ...
  test('delete confirmation modal + success notification flow', async ({
    page,
  }) => {
    await page.goto('/portfolio');
    ...
    // Click delete button (use last() to avoid strict mode violation with asset row)
    await page
      .getByRole('button', { name: /Delete ETH/i })
      .last()
      .click();
...
test.describe('Unusual Input Warnings (Phase 6)', () => {
  ...
  test('shows warning for tiny quantity (< 0.00000001) but allows proceed', async ({
    page,
  }) => {
    await page.goto('/portfolio');
  ...
  test('shows confirmation dialog when editing to large quantity', async ({
    page,
  }) => {
    await page.goto('/portfolio');
