...
test.describe('Educational Tooltips', () => {
  ...
  test.describe('Coin Detail Page - CoinMetrics', () => {
    test('shows tooltip on hover for Market Cap', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="coin-metrics"]', {
        timeout: 5000,
      });
      ...
      const helpIcon = marketCapLabel
        .locator('..')
        .getByRole('button', { name: /learn more about market_cap/i });
    ...
    test('shows tooltip on focus for Volume', async ({ page }) => {
      ...
      const helpIcon = volumeLabel
        .locator('..')
        .getByRole('button', { name: /learn more about volume/i });
  ...
  test.describe('Comparison Page - ComparisonTable', () => {
    test('shows tooltip for Market Cap in comparison table', async ({
      page,
    }) => {
      ...
      await page.waitForSelector(
        '[data-testid="comparison-table"], [data-testid="comparison-empty"]',
        { timeout: 5000 }
      );

      // If empty state, manually add a coin
      const isEmpty = await page
        .locator('[data-testid="comparison-empty"]')
        .isVisible()
        .catch(() => false);
      if (isEmpty) {
        // Use coin selector to add Bitcoin
        const coinSelector = page.locator(
          '[data-testid="coin-selector-search"]'
        );
        await coinSelector.fill('bitcoin');
        await page
          .getByRole('button', { name: /bitcoin/i })
          .first()
          .click();
      }
      ...
      const marketCapHelpIcon = comparisonTable
        .getByRole('button', { name: /learn more about market_cap/i })
        .first();
  ...
  test.describe('Portfolio Page - AssetMetricsPanel', () => {
    test('shows tooltip for metrics in expanded asset row', async ({
      page,
    }) => {
      ...
      await page.waitForSelector('[data-testid^="asset-expanded-container-"]', {
        timeout: 5000,
      });

      // Wait for metrics panel to load (it may show loading state first)
      await page.waitForSelector(
        '[data-testid="asset-metrics-panel"], [data-testid="asset-metrics-loading"]',
        { timeout: 10000 }
      );

      // If loading, wait for actual panel
      const isLoading = await page
        .locator('[data-testid="asset-metrics-loading"]')
        .isVisible()
        .catch(() => false);
      if (isLoading) {
        await page.waitForSelector('[data-testid="asset-metrics-panel"]', {
          timeout: 10000,
        });
      }
      ...
      const marketCapHelpIcon = metricsPanel
        .getByRole('button', { name: /learn more about market_cap/i })
        .first();
      await expect(marketCapHelpIcon).toBeVisible();
  ...
  test.describe('Market Overview Page', () => {
    test('shows tooltip for Market Cap metric card', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="metric-total-market-cap"]', {
        timeout: 5000,
      });
      ...
      const helpIcon = marketCapCard.getByRole('button', {
        name: /learn more about market_cap/i,
      });
      await expect(helpIcon).toBeVisible();
    ...
    test('shows tooltip for Volume metric card', async ({ page }) => {
      ...
      await page.waitForSelector('[data-testid="metric-total-volume"]', {
        timeout: 5000,
      });

      const volumeCard = page.getByTestId('metric-total-volume');
      const helpIcon = volumeCard.getByRole('button', {
        name: /learn more about volume/i,
      });
