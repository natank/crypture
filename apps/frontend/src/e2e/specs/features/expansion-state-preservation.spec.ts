...
test.describe("Asset Row Expansion State Preservation", () => {
  ...
  test.describe("Single Asset Expansion", () => {
    test("preserves single expanded asset on navigation", async ({ page, portfolioPage, addAssetModal }) => {
    ...
    test("collapsed asset remains collapsed after navigation", async ({ page, portfolioPage, addAssetModal }) => {
      // Add two assets
      await addAssetModal.openAndAdd('BTC', 1);
      ...
      await expect(portfolioPage.expandedContainerBySymbol('eth')).not.toBeVisible();
  ...
  test.describe("Multiple Asset Expansion", () => {
    test("preserves multiple expanded assets", async ({ page, portfolioPage, addAssetModal }) => {
      ...
      await expect(portfolioPage.expandedContainerBySymbol('btc')).toBeVisible({ timeout: 3000 });
      await expect(portfolioPage.expandedContainerBySymbol('eth')).toBeVisible({ timeout: 3000 });
    });
  });

  test.describe("Edge Cases", () => {
    test("sessionStorage is used (not localStorage)", async ({ page, portfolioPage, addAssetModal }) => {
      // Add and expand BTC
      await addAssetModal.openAndAdd('BTC', 1);
      await page.waitForTimeout(500);
