...
test.describe('Portfolio Composition Visualizations', () => {
    ...
    test.describe('Empty State', () => {
        test('should show empty state when portfolio has no assets', async () => {
            const compositionSection = portfolioPage.page.getByTestId('portfolio-composition-dashboard');

            // Empty state should be visible
            await expect(compositionSection.getByTestId('composition-empty-state')).toBeVisible();

            // Chart should not be visible
            await expect(compositionSection.getByTestId('allocation-pie-chart')).not.toBeVisible();
        });
    });
    ...
    test.describe('Asset Allocation View', () => {
        ...
        test.skip('should display tooltips on hover', async ({ page }) => {
            ...
            const compositionSection = page.getByTestId('portfolio-composition-dashboard');
            const chart = compositionSection.getByTestId('allocation-pie-chart');

            // Hover over a pie slice (implementation will add data-testid to slices)
            const btcSlice = chart.locator('[data-testid="pie-slice-bitcoin"]').first();
    ...
    test.describe('View Selector', () => {
        ...
        test('should have view selector with all options', async () => {
            const viewSelector = portfolioPage.page.getByTestId('allocation-view-selector');

            await expect(viewSelector).toBeVisible();
            await expect(viewSelector.getByRole('tab', { name: /asset/i })).toBeVisible();
            await expect(viewSelector.getByRole('tab', { name: /category/i })).toBeVisible();
            await expect(viewSelector.getByRole('tab', { name: /market cap/i })).toBeVisible();
            await expect(viewSelector.getByRole('tab', { name: /risk/i })).toBeVisible();
        });
        ...
        test('should switch to market cap tier view', async () => {
            const viewSelector = portfolioPage.page.getByTestId('allocation-view-selector');
            const marketCapTab = viewSelector.getByRole('tab', { name: /market cap/i });
        ...
        test('should switch to risk level view', async () => {
            ...
            const hasConservative = await legend.getByText(/conservative/i).isVisible().catch(() => false);
            const hasModerate = await legend.getByText(/moderate/i).isVisible().catch(() => false);
            const hasAggressive = await legend.getByText(/aggressive/i).isVisible().catch(() => false);
    ...
    test.describe('Accessibility', () => {
        ...
        test('should support keyboard navigation for view selector', async ({ page }) => {
