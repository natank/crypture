...
export class MarketPage {
  ...
  constructor(page: Page) {
    ...
    // Trending & Discovery
    this.trendingSection = page.getByRole('heading', {
      name: /trending coins/i,
    });
    this.topGainersSection = page.getByRole('heading', {
      name: /top gainers/i,
    });
    ...
    this.trendingCoins = page
      .locator('text=Trending Coins')
      .locator('..')
      .locator('.grid > div');

    this.topGainers = page
      .locator('text=Top Gainers')
      .locator('..')
      .locator('.space-y-4 > div');
    this.topLosers = page
      .locator('text=Top Losers')
      .locator('..')
      .locator('.space-y-4 > div');
  }
  ...
  async getMetricValue(metric: Locator): Promise<string> {
    return (
      (await metric.locator('[data-testid="metric-value"]').textContent()) || ''
    );
  }

  async getMetricChange(metric: Locator): Promise<string> {
    return (
      (await metric.locator('[data-testid="metric-change"]').textContent()) ||
      ''
    );
  }
