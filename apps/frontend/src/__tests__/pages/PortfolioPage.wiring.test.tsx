...
vi.mock('@hooks/usePortfolioState', () => ({
  usePortfolioState: () => ({
    portfolio: [
      { coinInfo: { id: 'btc', symbol: 'btc', name: 'Bitcoin' }, quantity: 1 },
    ],
    addAsset: vi.fn(),
    removeAsset: vi.fn(),
...
vi.spyOn(hooks, 'usePortfolioImportExport').mockImplementation(
  () => mockHook as any
);
...
describe('PortfolioPage wiring', () => {
  ...
  it('shows general error banner when coin list has an error', () => {
    ...
    render(<PortfolioPage />);
    expect(
      screen.getByText(/Error loading prices. Please try again later\./i)
    ).toBeInTheDocument();
  });
