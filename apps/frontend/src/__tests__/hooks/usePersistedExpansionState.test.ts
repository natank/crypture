...
describe('usePersistedExpansionState', () => {
  ...
  it('initializes with empty expansion state', () => {
    const { result } = renderHook(() => usePersistedExpansionState(['bitcoin', 'ethereum']));
  ...
  it('loads from sessionStorage on mount', () => {
    sessionStorage.setItem('portfolio_expansion_state', JSON.stringify({
      expandedAssets: ['bitcoin', 'ethereum']
    }));
    
    const { result } = renderHook(() => usePersistedExpansionState(['bitcoin', 'ethereum', 'cardano']));
  ...
  it('handles invalid sessionStorage data gracefully', () => {
    ...
    const { result } = renderHook(() => usePersistedExpansionState(['bitcoin']));
