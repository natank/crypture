...
describe('CoinSelector', () => {
  it('renders search input', () => {
    render(
      <CoinSelector coins={mockCoins} onSelect={vi.fn()} excludeIds={[]} />
    );

    expect(screen.getByTestId('coin-selector-input')).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText('Search coins to add...')
    ).toBeInTheDocument();
  });
  ...
  it('calls onSelect when a coin is clicked', () => {
    ...
    render(
      <CoinSelector coins={mockCoins} onSelect={onSelect} excludeIds={[]} />
    );
  ...
  it('clears search and closes dropdown after selection', () => {
    ...
    expect(
      screen.queryByTestId('coin-selector-results')
  ...
  it('limits results to 10 items', () => {
    ...
    render(
      <CoinSelector coins={manyCoins} onSelect={vi.fn()} excludeIds={[]} />
    );
