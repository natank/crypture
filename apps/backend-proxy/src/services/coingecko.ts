...
export class CoinGeckoService {
  ...
  constructor() {
    if (!process.env.COINGECKO_API_KEY) {
      console.warn('⚠️  COINGECKO_API_KEY not found in environment variables. Using free tier with rate limits.');
    }
    ...
    this.api.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 429) {
          console.warn('⚠️  Rate limit exceeded. Consider upgrading to CoinGecko Pro API.');
        }
        return Promise.reject(error);
      }
    );
  ...
  async getSimplePrice(ids: string, currencies: string = 'usd'): Promise<CoinGeckoSimplePrice> {
  ...
  async getRateLimitInfo(): Promise<{ limit: number; remaining: number } | null> {
    try {
      const response = await this.api.get('/simple/price?ids=bitcoin&vs_currencies=usd');
