...
echo "🔧 Setting up Testcontainers with Podman for backend-proxy..."
echo ""
...
if [ -n "$SHELL_RC" ]; then
    ...
    if ! grep -q "TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE" "$SHELL_RC"; then
        echo "" >> "$SHELL_RC"
        echo "# Testcontainers + Podman configuration (backend-proxy)" >> "$SHELL_RC"
...
echo "Step 5: Creating test runner scripts..."
...
# Create run-container-tests.sh
cat > "$SCRIPT_DIR/run-container-tests.sh" << 'EOF'
#!/bin/bash
...
chmod +x "$SCRIPT_DIR/run-container-tests.sh"
echo "✅ Created run-container-tests.sh"
...
echo "  📄 $SCRIPT_DIR/run-tests.sh (integration tests)"
echo "  📄 $SCRIPT_DIR/run-container-tests.sh (container tests)"
echo "  📄 $SCRIPT_DIR/run-all-tests.sh (all tests)"
echo ""
echo "To run tests NOW (in this terminal):"
echo "  cd $(dirname "$(dirname "$SCRIPT_DIR")")"
echo "  ./tests/integration/run-tests.sh"
echo ""
echo "To run container tests:"
echo "  ./tests/integration/run-container-tests.sh"
echo ""
echo "To run all tests:"
echo "  ./tests/integration/run-all-tests.sh"
echo ""
echo "To run tests in NEW terminal (after restarting terminal):"
echo "  source $SHELL_RC"
echo "  npm run test:containers:integration"
echo ""
echo "Or reload your shell profile now:"
echo "  source $SHELL_RC"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
