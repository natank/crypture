...
test.describe("Edit Asset Quantity", () => {
  ...
  test("should edit asset quantity via inline edit", async ({ page }) => {
    ...
    await dialog.locator("select#asset-select").selectOption({ label: "Bitcoin (BTC)" });
  ...
  test("should cancel edit and restore original quantity", async ({ page }) => {
    ...
    await dialog.locator("select#asset-select").selectOption({ label: "Ethereum (ETH)" });
    ...
    await page.getByRole("button", { name: /Cancel editing/i }).last().click();
  ...
  test("should display validation error for negative numbers", async ({ page }) => {
    await page.getByRole("button", { name: /add asset/i }).click();
    const dialog = page.getByRole("dialog");
    await expect(dialog).toBeVisible();
    await dialog.getByLabel("Filter assets").fill("Bitcoin");
    ...
    await expect(page.getByText(/Quantity must be greater than zero/i)).toBeVisible();

    const saveButton = page.getByRole("button", { name: /Save changes/i }).last();
  ...
  test("should display validation error for too many decimals", async ({ page }) => {
    await page.getByRole("button", { name: /add asset/i }).click();
    const dialog = page.getByRole("dialog");
    await expect(dialog).toBeVisible();
    await dialog.getByLabel("Filter assets").fill("Bitcoin");
    ...
    await expect(page.getByText(/Maximum 8 decimal places allowed/i)).toBeVisible();
  });
  ...
  test("should handle keyboard navigation", async ({ page }) => {
    ...
    await page.keyboard.press("Tab");
    const cancelButton = page.getByRole("button", { name: /Cancel editing/i }).last();
...
test.describe("Edit Asset Quantity - Mobile", () => {
  ...
  test("should have touch-friendly edit controls on mobile", async ({ page }) => {
    mockCoinGeckoMarkets(page);
    mockCoinGeckoChartData(page);
    await page.goto("/portfolio");
    await expect(page.getByText(/Your Assets/i)).toBeVisible();
