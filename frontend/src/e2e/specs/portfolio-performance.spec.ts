...
test.describe('Portfolio Performance Chart', () => {
    test('should display the portfolio performance chart', async ({ portfolioPage, addAssetModal }) => {
        // Add an asset to ensure portfolio has data
        await addAssetModal.openAndAdd('BTC', 1);

        // Verify chart is visible
        await expect(portfolioPage.page.getByTestId('portfolio-performance-chart')).toBeVisible();
    });

    test('should allow switching time ranges', async ({ portfolioPage, addAssetModal }) => {
        // Add an asset to ensure portfolio has data
        await addAssetModal.openAndAdd('BTC', 1);

        // Verify time range selector is visible
        await expect(portfolioPage.page.getByTestId('time-range-selector')).toBeVisible();
    ...
    test('should display performance metrics', async ({ portfolioPage, addAssetModal }) => {
        // Add an asset to ensure portfolio has data
        await addAssetModal.openAndAdd('BTC', 1);
        ...
        await expect(portfolioPage.page.getByTestId('portfolio-value-change')).toBeVisible();
        await expect(portfolioPage.page.getByTestId('portfolio-percent-change')).toBeVisible();
    });

    test('should show tooltip on hover', async ({ portfolioPage, addAssetModal }) => {
        // Add an asset to ensure portfolio has data
        await addAssetModal.openAndAdd('BTC', 1);
