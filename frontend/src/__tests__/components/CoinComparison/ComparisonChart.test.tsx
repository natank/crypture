...
describe("ComparisonChart", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (coinService.fetchAssetHistory as ReturnType<typeof vi.fn>).mockResolvedValue(mockPriceHistory);
  });
  ...
  it("renders nothing when no coins and not loading", () => {
    const { container } = render(<ComparisonChart coins={[]} isLoading={false} />);
  ...
  it("renders chart section with heading", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Price Performance" })).toBeInTheDocument();
  });
  ...
  it("fetches chart data for each coin", async () => {
    render(<ComparisonChart coins={[mockBitcoin, mockEthereum]} isLoading={false} />);

    await waitFor(() => {
      expect(coinService.fetchAssetHistory).toHaveBeenCalledWith("bitcoin", 30);
      expect(coinService.fetchAssetHistory).toHaveBeenCalledWith("ethereum", 30);
    });
  });
  ...
  it("shows error message when fetch fails", async () => {
    (coinService.fetchAssetHistory as ReturnType<typeof vi.fn>).mockRejectedValue(
  ...
  it("has accessible time range group", async () => {
    ...
    expect(screen.getByRole("group", { name: "Time range selection" })).toBeInTheDocument();
  });
});
